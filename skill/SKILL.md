...
description: 在安卓手机上通过 Termux 完整部署 OpenClaw AI 网关（原生方案，无需 root/proot），覆盖 SSH 远程管理、模型供应商配置、runit 进程保活、Termux:Boot 开机自启、Tailscale 固定 IP、adb 系统加固、QQ 机器人渠道接入，内置 14 个实战踩坑的修复方案。当用户要求在手机/旧手机/安卓设备上部署 OpenClaw、把手机改造成 AI 服务器/低功耗服务器、复刻 openclaw-termux 部署、或排查手机上已部署 OpenClaw 的故障（掉线/不回话/重启失联）时使用。
---
...
[references/pitfalls.md](references/pitfalls.md)（14 个已知坑的现象→解法速查）。
