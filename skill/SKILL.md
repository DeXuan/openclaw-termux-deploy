...
description: 在安卓手机上通过 Termux 完整部署 OpenClaw AI 网关（原生方案，无需 root/proot），覆盖 SSH 远程管理、模型供应商配置、runit 进程保活、Termux:Boot 开机自启、Tailscale 固定 IP、adb 系统加固、QQ/飞书机器人渠道接入，内置 16 个实战踩坑的修复方案和 4 台真机（Android 7/10/15）的机型适配矩阵。当用户要求在手机/旧手机/安卓设备上部署 OpenClaw、把手机改造成 AI 服务器/低功耗服务器、复刻 openclaw-termux 部署、或排查手机上已部署 OpenClaw 的故障（掉线/不回话/重启失联）时使用。
---
...
[references/pitfalls.md](references/pitfalls.md)（16 个已知坑的现象→解法速查）。

**机型差异**：动手前先 `getprop ro.product.model ro.build.version.release`，
按 [references/device-matrix.md](references/device-matrix.md) 的 Android 版本决策树取该机的加固/网络方案
（K60/MIX 2S/Note 7/Note 4X 四台真机已验证，覆盖 Android 7/10/15）。
...
脚本已内置坑 2（`GYP_DEFINES="android_ndk_path="`）和坑 3（npm `--allow-scripts`）的修复。
