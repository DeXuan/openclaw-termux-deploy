...
- [功能详解](#功能详解)
  ...
  - [8. 模型管理](#8-模型管理)
  - [9. 渠道部署 — 打通 QQ/飞书/微信](#9-渠道部署--打通-qq飞书微信)
- [非交互模式](#非交互模式)
- [常见问题](#常见问题)
- [进阶玩法](#进阶玩法)
...
```
  ...
  6   🩺 自愈系统          安装/配置/状态/日志
  7   🧩 技能工具箱        安装/搜索/同步技能
  8   🤖 模型管理          模型列表/渠道状态/免费额度/额度切换
  9   📡 渠道部署          安装/配置 QQ/飞书/微信机器人
...
额度自动切换：当免费模型额度耗尽返回 403 时，watcher 自动切换到下一个有额度的模型。部署后加入 `~/.termux/boot/start-services.sh` 可开机自启。

### 9. 渠道部署 — 打通 QQ/飞书/微信

**工具箱最核心的集成功能。** 一键安装 + 配置三种聊天渠道：
...
1. 自动安装 `@openclaw/qqbot` 插件
2. 提示去 https://q.qq.com 注册，获取 AppID 和 AppSecret
3. 自动显示当前出口 IP（方便填白名单）
4. 一键 `openclaw channels add` 配置
5. 自动重启 gateway 生效

> ⚠️ QQ 必须配置 IP 白名单，宽带重拨后需更新。白名单加好无需重启，插件每分钟自动重试。

#### 飞书机器人

1. 自动安装 `@openclaw/feishu` 插件
2. 配置 AppID/AppSecret
3. 自动重启生效

> ✅ 飞书 WebSocket 长连接，**无 IP 白名单**，全队最省心。

#### 微信 iLink

1. 自动安装 `which` 依赖（微信安装器必需）
2. 安装 `@tencent-weixin/openclaw-weixin` 插件
3. 扫码绑定——支持两种模式：
   - **本机扫码**：Termux 内直接显示二维码
   - **远程链接**：后台启动登录 → 提取 `liteapp.weixin.qq.com` 链接 → 发到微信打开
...
| `./openclaw-deploy install K60` | 远程部署到 K60 | 批量部署多台设备 |
...
├── openclaw-deploy         ← 🚀 工具箱入口 (bash TUI, 68 行)
├── lib/
│   ├── common.sh           ← UI 组件库 / 设备配置 / SSH (267 行)
│   └── menus.sh            ← 9 个菜单函数 + 辅助函数 (747 行)
├── scripts/
...
├── docs/
│   └── device-comparison.md ← 机队全景文档
├── skill/                   ← OpenClaw 部署技能
├── skill-formatter/         ← 微信公众号格式化技能
├── bailian-quota-switcher/  ← 百炼免费额度自动切换
└── GUIDE.md                 ← 📖 本文档
```
